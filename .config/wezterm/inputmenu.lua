...
local chooseWorkspace = wezterm.action_callback(function(window, pane, id, label)
	...
	window:perform_action(
		wezterm.action.SwitchToWorkspace({
			name = id,
			spawn = workspaces[id] and workspaces[id].spawn,
		}),
		pane
	)
end)
...
local inputmenu = wezterm.action_callback(function(window, pane)
	...
	for id, workspace in pairs(workspaces) do
		...
		local label = string.format("%s %s", workspace.icon or "$", workspace.spawn.label)

		if not isNameInMuxspaces then
			table.insert(choices, { id = id, label = label })
		else
			local is_current_workspace = current_workspace == id
			table.insert(choices, { id = id, label = format_active(label, is_current_workspace) })
		end
	end
	...
	for _, name in ipairs(muxspaces) do
		...
		local label = string.format("%s %s", "$", name)
		local formatted_label = name == current_workspace and format_current(label) or label
		if not isNameInChoices then
			table.insert(choices, { id = name, label = formatted_label })
		end
	end
